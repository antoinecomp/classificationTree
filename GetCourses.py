...
class GetCourses:
	def __init__(self):
		...
		if elegido[1] == "---":
			...
			self.df_query = df.loc[elegido[0]]
			print("You selected :\n",df.loc[elegido[0]])
		elif elegido[2] == "---":
			self.df_query = df.loc[elegido[0],elegido[1]]
			print("You selected :\n",df.loc[elegido[0],elegido[1]])
		elif elegido[3] == "---":
			self.df_query = df.loc[elegido[0],elegido[1],elegido[2]]
			print("You selected :\n",df.loc[elegido[0],elegido[1],elegido[2]])
		else:
			self.df_query = df.loc[elegido[0],elegido[1],elegido[2],elegido[3]]
